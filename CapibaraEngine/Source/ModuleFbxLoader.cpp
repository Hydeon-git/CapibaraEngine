...
void ModuleFbxLoader::LoadFile(const char* filePath, std::vector<MeshComponent>& meshDataVec)
{
	...
	if ((scene != nullptr) && (scene->HasMeshes()))
	{
		...
		for (int i = 0; i < scene->mNumMeshes; i++)
		{
			...
			meshData.numMeshes = scene->mNumMeshes;

			meshData.numVertex = sceneMesh->mNumVertices;
			meshData.vertex = new float[meshData.numVertex * 3];
			memcpy(meshData.vertex, sceneMesh->mVertices, sizeof(float) * meshData.numVertex * 3);
			...
			{
				meshData.numIndex = sceneMesh->mNumFaces * 3;
